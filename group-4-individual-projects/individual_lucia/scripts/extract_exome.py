...
def extract_weg(data,n):
        sig.sigProfilerExtractor(
        "vcf",
        "individual_lucia/wEg"+str(n),
        data,
...
"""
    Run the exome for multiple value of n to find best number of signatures on exome dats
"""
...
def build_csv():
    output_file = 'individual_lucia/aggregated_exome_solutions.csv'
    with open(output_file, 'a',newline='') as final_csv:
        writer = csv.writer(final_csv)

        # Save header
        with open("individual_lucia/wEg1/SBS96/All_solutions_stat.csv", 'r') as output_csv:
            reader = csv.reader(output_csv)
            writer.writerow(next(reader))

        # Look up solution statistics
        for i in range(1, 16):
            path = "individual_lucia/wEg"+str(i)+"/SBS96/All_solutions_stat.csv"
            # print(path)
...
def plot_lines(signatures, reproducibility, frobenius):
    ...
    plt.savefig("individual_lucia/Exome_convergence.pdf")


if __name__ == "__main__":

    vcf_data =  "individual_lucia/exome_data/"

    # Find N
    # signatures_csv = run_for_all_signature_numbers(vcf_data)
    signatures_csv = 'individual_lucia/aggregated_exome_solutions.csv'
